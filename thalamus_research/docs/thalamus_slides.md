...
The `..` subpackage is THALAMUS's scientific layer. It is never loaded at query time and has no impact on production latency. Its purpose is to evaluate how well THALAMUS performs versus alternative selection strategies — and to generate the data needed to publish that evaluation.
...
**Separation principle:** `thalamus-research` is a separate CLI entry point from `thalamus-select`. Research imports live entirely under `..`. The four production packages (`component_scoring/`, `oracle_builder/`, `context_selectors/`, `shared/`) have no dependency on `..`.
